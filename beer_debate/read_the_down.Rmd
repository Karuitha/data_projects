...
3. The most popular beer category by median ratings is `IPA - New England` followed closely by `Sour - Fruited Kettle Sour` and `Wild Ale`.

4. `Tree House Brewing Company` has the most brands of beer featuring in the dataset.

4. Beers made by `Anchorage Brewing Company` have the highest median rating.

3. When it comes with beers with weird names, there is no shortage. Sample these:
...
There are several categories of beer that each of the beers in the data belongs to. Overall, there are 83 beer categories. The variable `type` gives these classifications. The most common beer categories include. Note that I also filter out beers with less than 100 votes. 
