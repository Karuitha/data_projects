...
admission_daily <- read_csv(file = "data/ae_uk.csv") |> 
    mutate(arrival_time= dmy_hm(arrival_time)) |> 
    dplyr::distinct() |> 
    as_tsibble(index = arrival_time, key = c(gender,type_injury), regular=FALSE) |> 
    index_by(time = lubridate::as_date(arrival_time)) %>% 
...
admission_fit <- admission_daily |>
  model(
    ...
    exponential_smoothing = ETS(arrival), # exponential_smoothing
    arima = ARIMA(arrival), #ARIMA
    regression = TSLM(arrival) # Time series linear regression
  )
```
...
fit <- admission_fit |> augment() |> filter(.model=="snaive")


fit |> autoplot(arrival) + autolayer(fit,.fitted, color="red", lty=3)
```
...
test <- admission_daily %>%
  ...
  filter_index(as.character(max(admission_daily$time)-round(percentage_test*length(admission_daily$time))+1) ~ .)

 

train <- admission_daily %>%

  filter_index(. ~ as.character(max(admission_daily$time)-(round(percentage_test*length(admission_daily$time)))))
```

2. Use `stretch_tsibble()` to Create different time-series (i.e different id)

We apply time series cross validation on the train data. We first start with an initial training size (.init = ) and then increase the size of the previous time series by adding more new observation(.step=) to create a new time series, we continue creating these time series until the number of observation left at the end of time series equals to the forecast horizon, we stop there.
...
ae_model_tscv <- train_tscv |>
  model(
    ...
    exponential_smoothing = ETS(arrival),
    arima = ARIMA(arrival),
...
ae_model_tscv |> 
    filter(.id==60) |> 
    glance() # if you want to observe one specific .id
...
```{r label, options}
fc_accuracy_by_id <- ae_fcst_tscv |>
  accuracy(admission_daily, by = c(".id", ".model"))
```

We can now create some insightful visualisations:

```{r label, options}
fc_accuracy_density <- fc_accuracy_by_id |> 
...
#only point forecast
 ae_fcst_tscv |>
  accuracy(train,measures = list(point_accuracy_measures)) 
#only prediction interval forecast using winkler score
ae_fcst_tscv |> 
  accuracy(train, measures = list(interval_accuracy_measures))
