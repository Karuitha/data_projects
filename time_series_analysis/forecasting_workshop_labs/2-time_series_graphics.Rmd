...
total_arrival_hourly <- arrival_tsb %>%  
  index_by(time = lubridate::floor_date(arrival_time, "1 hour")) %>%
  summarise(arrival= n(), .groups = "drop") %>% fill_gaps(arrival=0)
...
total_arrival_hourly %>% gg_tsdisplay()#  check ?gg_tsdisplay
...
###################################
library(tidyquant)
mult_stocks <- tq_get(c("GOOG", "AMZN"),
                      get  = "stock.prices",
                      from = "2016-01-01",
                      to   = "2017-01-01") %>% 
    select(date, symbol, adjusted)

plotly::ggplotly(mult_stocks %>% ggplot(aes(x = date, y = adjusted, color = symbol)) + 
