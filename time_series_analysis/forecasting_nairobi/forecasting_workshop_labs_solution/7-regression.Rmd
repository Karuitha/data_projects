...
for the regression model, we use predictors such as holidays and temperature. How do we know which predictor to examine?  ask someone with the knowledge of the context. Someone who is knowledgeable in the domain can tell you which predictors might be useful in your forecasting case. You can try those predictors in the model and see if they are useful.

You can also use visualizations, but before that the most important thing is the domain knowledge. Here we explore using daily average temperature and few different public holidays:

so we need to import them and do a bit of data manipulation to prepare them for forecasting:

```{r predictors}
temp <- read_csv("data/temp.csv") %>% 
  rename(average_daily_temperature=temp) #import the temperature data
holiday <- read_csv(
  file="data/holiday.csv",
  col_types = cols(date=col_date(format = "%d/%m/%Y"))) %>% 
  mutate_if(is_double, factor) %>% mutate(date=as_date(date)) %>% 
  janitor::clean_names()

tem_holiday <- temp %>% left_join(holiday, by="date")
tem_holiday_tsbl <- tem_holiday %>% as_tsibble(index = date)
```

We need this to join datasets and create a single tsibble with all data we need for regression modeling:

```{r label, options}
admission_regression <- tem_holiday_tsbl %>% inner_join(admission_daily, by="date")
```

when doing time series forecasting, we need to have always one tsibble containing all variables for the modeling, so that is why we join both admission_daily and predictors to create one single tsibble.
...
```{r train}
fit_regression <- train %>% filter(date< (max(date)-f_horizon+1)) %>% model(
  ...
  arima_x = AR(arrival ~ average_daily_temperature+black_friday+boxing_day+christmas_day+halloween_day+new_years_day)
)
```
...
tarin_tscv <- train %>% filter(date<(max(date)-f_horizon+1)) %>%
  stretch_tsibble(.init = 4*365, .step = 7) # I have changed .step=7 to shorter the running time, but .step should be =1 or the forecasting frequency 

# you need also to get future values that correspond to each .id, because you need them in the forecast model:
m_date <- train %>% pull(date)
test_tscv <- train %>% filter(date>m_date[4*365]) %>% 
...
fcs_ets_temperature_tscv <- tarin_tscv %>% model(ets=ETS(average_daily_temperature)) %>% 
...
# Here I show how we produce estimation of the temperature for the future in case we need it to use (in our exampel, we don't need it because the best model is regresssion withholidays)
