...
admission_fit <- admission_daily |>
  model(
    ...
    regression = TSLM(arrival),#time series linear regression models
  )
```
...
admission_fit |> augment() |> filter(.model=="snaive") |> as_tibble() |> select(.fitted)#select fitted values
...
```{r plot-forecast}
admission_fc |> autoplot(admission_daily, level=NULL)
```
...
test <- admission_daily |> 
  filter_index(as.character(max(admission_daily$date)-round(percentage_test*length(admission_daily$date))+1) ~ .)

train <- admission_daily |>
  filter_index(. ~ as.character(max(admission_daily$date)-(round(percentage_test*length(admission_daily$date)))))
...
train_tscv <- admission_daily |> # split data into series with increasing size
  filter_index(. ~ as.character(max(admission_daily$date)-(f_horizon))) |>
  stretch_tsibble(.init = length(train$date), .step = 7)
```
...
ae_fcst_tscv |> 
  accuracy(admission_daily,measures = list(interval_accuracy_measures))
