...
In this step, I create a training and testing set. The training set has 75% of the data, while the testing set has the remaining 25% of the data points. 

```{r}
## Max Kuhn is leading the tidymodels project
library(tidymodels)
...
## multicollinearity 
## Unstable coefficients. The model is not stable. 
```
...
broom::tidy(logistic_results)

stargazer::stargazer(glm(left ~ ., data = df_baked_train, family = "binomial"), type = "html", title = "Results of Logistic Regression", align = TRUE)
```
